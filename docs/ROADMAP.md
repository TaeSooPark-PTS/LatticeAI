...
As of **2026-08-18**, against the 12.0.0 — Open House tree. The rows below
...
| **Worker-side batch embed** | **Mostly closed in 12.0.0 on the drain path**: embedding moved ahead of the write transaction, so the writer no longer holds a lock while a model runs — ~66 → ~1,300 items/s, and a 991-item backlog drains in 15.3s instead of ~40 minutes. What remains is the ingest-time door: it batches only when `(model_id, dim)` already agrees, and the hash path still embeds inline, one node at a time. | M | **P1** — the backlog path is fast now; a real-embed default still needs the inline door not to serialise against the single MLX executor. | Make `GraphWriter::write_vectors_with` + one `/worker/embed` call the ingest-time path whenever a provider is bound. Keep the drain planner as the backlog path. Do not open a second embedder inside `lattice-core`. |
