...
rebuild (`cargo build -p lattice-host --release`) if `-V` is not 12.0.0.
...
included) must carry `Current release: **12.0.0 — …**` matching
...
`output/release/v12.0.0/SCREENSHOT_INDEX.md` still names the live
...
bumps. Do not move this file's `Current release` marker off 12.0.0 ahead of
that lane.

Release/publish examples must use exact target-version filenames. Do not
document wildcard artifact upload commands.

For 12.0.0 release work, exact artifacts are:
