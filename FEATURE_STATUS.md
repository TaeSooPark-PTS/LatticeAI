# Lattice AI Feature Status (v11.9.0)
...
deleted in the 11.6.0 port — is restored. What is still open, including 2B
agent-run *quality* and the mock-only `api_key` path, is named in Known
Limitations.
...
| Computer-use Status | Current | Status is a real worker capabilities probe (`pointer_tools` on `/worker/sysinfo`, additive with `python_version`). The six pyautogui pointer tools still execute in the worker; a stock install answers unavailable. |
| Tool Registry / MCP | Current | ToolRegistry diagnostics, explicit desktop/knowledge/network policy gates, and MCP install state are separated from app-factory helpers. **11.9.0:** `POST /mcp` is a real streamable-HTTP JSON-RPC server (`initialize` / `tools/list` / `tools/call`) exposing a curated safe tool set plus seven skills with parsed schemas; governance refusals are JSON-RPC errors; `/mcp/call` really dispatches; `/mcp/install` is honest (enables skills/plugins, remotes stay manual). `/mcp` is outside the OpenAPI product contract by design. `build` / `deploy_project` really run governed scripts. |
...
| Release Assets | Current | 11.9.0 package metadata, static app, release notes, current documentation, and exact artifact names are aligned. The macOS dmg is ad-hoc signed (effectively unsigned), as in every release so far. |

## Known Limitations

- **2B agent-loop content quality is honestly gated.** gemma-4-e2b writes
  the requested file mechanically (5/5 in the closing tape) but its
  summaries can fail the critic and end FAILED/NEEDS_REVIEW. The funnel
  advises a larger model for agent runs.
- **The `api_key` cloud path is mock-verified only.** It is contract-tested
  ...
  budget for that path. Live OAuth E2E used `cli_oauth` (`agy` / `grok`)
  at zero API billing.
- **Restore in a long-lived process may serve pre-restore bytes** until
  connections recycle. Restart after restore.
- **brew/pip setup items stay manual by design.** `/setup/install` says so
  rather than pretending to run them.
- **`/mcp` is outside the OpenAPI product contract by design.** The
  JSON-RPC server at `POST /mcp` is real.
- **The macOS dmg remains ad-hoc signed** (effectively unsigned). First
  launch needs the usual Gatekeeper step.
...
- **A vault watch re-runs the whole sync, it does not diff notes.** The 11.1.0
  objection stands — link edges need the node ids only a completed inline
  ingest has — so watch mode uses the mtime snapshot to decide *whether* to
  sync, then runs the full bridge (idempotent by content hash). It is opt-in
  and off by default (`LATTICEAI_VAULT_WATCH`, the injectable gate, or the home
  dock's 기능 drawer). A large
  vault is still capped at 2,000 notes per run and reports `truncated: true`;
  only frontmatter `tags` become topics, inline `#tags` are not parsed.
