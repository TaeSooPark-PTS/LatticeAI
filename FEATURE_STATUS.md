# Lattice AI Feature Status (v12.2.1)
...
| Cloud Models | Opt-in | Cloud prompts are sent only after credentials resolve and the user (or the escalation policy) selects a cloud path. Dual modes since 11.9.0: `api_key` (OpenAI-compatible; 12.2.1 live-probes `GET /models` and fail-closes if unreachable) and `cli_oauth` (locally OAuth-authenticated `agy` / `grok`). See Hybrid Cloud Chat. |
...
- **Small-model *content* quality is honestly gated.** The mechanical half is
  ...
  **12.2.1** fills a thin summary from the file that was actually read;
  if those words are still missing the run is `NEEDS_REVIEW`, not a false
...
- **The `api_key` cloud path probes `GET /models`.** No completion is
  billed to verify a key. An unreachable provider or a rejected key
  fail-closes status. Live chat completions still need a budget; live
  OAuth E2E used `cli_oauth` (`agy` / `grok`) at zero API billing.
...
- **Watch prune is graph-only.** A vanished watched file is removed from
  the Brain; disk is never deleted. A one-shot folder ingest still
  reports vanished files and waits for
  `POST /api/ingestion/folder/prune` (dry-run, then `confirm`).
