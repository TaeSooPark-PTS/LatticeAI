...
Last updated for the 6.3.0 Product Hardening Completion target.
...
```mermaid
flowchart TB
  Source["Source Tree 6.3.0"] --> FrontendBuild["Vite Frontend Build"]
  ...
  PythonBuild --> Wheel["dist/ltcai-6.3.0-py3-none-any.whl"]
  PythonBuild --> Sdist["dist/ltcai-6.3.0.tar.gz"]
  NpmPack --> Tgz["ltcai-6.3.0.tgz"]
  VsixBuild --> Vsix["dist/ltcai-6.3.0.vsix"]
  TauriBuild --> Dmg["src-tauri/target/release/bundle/dmg/Lattice AI_6.3.0_aarch64.dmg"]
  StaticAssets --> Wheel
  StaticAssets --> Tgz
  StaticAssets --> Dmg
```
...
- Historical reports under `docs/` preserve older release behavior and should
  not be rewritten as 6.3.0 claims.
