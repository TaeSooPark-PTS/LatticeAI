...
**11.9.0 — Working Order** keeps the One Door topology (`lattice-host`, 420 operations / 41 families, 19-route worker) and makes the remaining Current stubs and the half-wired lanes actually run. Hybrid cloud is optional (default local; OAuth CLI `agy` / `grok` supported; `api_key` mock-verified only). MCP is a real JSON-RPC server at `POST /mcp`. Chat file generation is restored. Full disclosure of what is still open — 2B agent-run quality, the mock-only `api_key` path, restore needing a restart, brew/pip staying manual, `/mcp` outside the OpenAPI contract, the ad-hoc-signed dmg — is in `RELEASE_NOTES_v11.9.0.md`.

**The extension surface is unchanged apart from its version.** Sync, status, recall, staged proposals, `POST /agent` with `stream:true`, and `strict` / `trusted` / `bypass` travel over the wire as before. Cloud escalation and MCP live on the host, not in the editor.
