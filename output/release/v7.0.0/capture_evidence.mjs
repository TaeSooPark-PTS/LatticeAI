...
async function main() {
  ...
  await page.locator("input").nth(0).fill("Codex");
  await page.getByRole("textbox", { name: "you@local", exact: true }).fill("codex@local");
  await page.locator("input").nth(2).fill("Lattice123");
  await page.getByRole("button", { name: "내 Brain 시작하기" }).click();
  await page.getByText("이 컴퓨터에서 가능한 경험을 확인합니다.").waitFor();
  ...
  await page.getByText(/Review Center|리뷰 센터/).first().waitFor();
  await shot(page, "13-review-center.png");
...
| [13-review-center.png](screenshots/13-review-center.png) | Automation Review Center |
