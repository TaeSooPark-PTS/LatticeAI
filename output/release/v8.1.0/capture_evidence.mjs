...
async function main() {
  ...
  await page.getByRole("button", { name: "기억 그래프", exact: true }).click();
  ...
  await page.getByRole("button", { name: "모델 선택", exact: true }).click();
  ...
  await page.getByText("Approve 8.1 product readiness evidence").waitFor();
