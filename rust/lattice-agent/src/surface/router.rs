...
//!   [`crate::tools::exec::NATIVE_EXECUTABLES`] names. Everything else comes back as a
...
use crate::kernel::permission::{block_reason_for_tool, non_auto_plan_steps, PlanStep};
use crate::kernel::policy::{PolicyTable, ToolPolicy};
use crate::surface::bad_request;
use crate::tools::command::validate;
use crate::tools::exec::{execute, is_natively_executable};
use crate::tools::sandbox::{Workspace, MAX_COMMAND_SECONDS};
...
async fn contract(Query(query): Query<ContractQuery>) -> Response {
    ...
    Json(json!({
        "mode": mode.as_str(),
        "requested": requested,
        "default_mode": crate::kernel::mode::DEFAULT_MODE.as_str(),
        "contract": mode_contract(mode),
        "modes": ALL_MODES
            .iter()
