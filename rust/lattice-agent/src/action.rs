...
pub fn extract_json_object(raw: &str) -> Result<(Value, Vec<String>), ActionError> {
    ...
    if !text.starts_with('{') {
        ...
        if let Some(payload) = crate::channel::strip_channel_frames(&untagged) {
            untagged = payload;
        }
            ...
            match pyjson::loads(&repaired) {
                ...
                Err(error) => {
                    ...
                    match closed {
                        ...
                        None => match crate::pyliteral::literal_eval(&text) {
...
fn salvage_labeled_action(raw: &str) -> Option<Parsed> {
    ...
    let text = match crate::channel::strip_channel_frames(stripped.trim()) {
        Some(payload) => {
            repairs.push("tag_strip".into());
            payload
        }
