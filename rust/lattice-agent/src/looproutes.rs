...
pub struct LoopConfig {
    ...
    /// Where paused runs are persisted. See [`crate::runs`] for why this is not
    /// Python's directory.
...
async fn run(State(state): State<Arc<LoopState>>, Json(raw): Json<Value>) -> Response {
    ...
    let observer: crate::agentloop::StepObserver = Box::new(move |event: Value| {
