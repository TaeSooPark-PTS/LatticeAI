...
//!
//! Nothing here spawns anything. [`validate`] returns a verdict; [`crate::exec`]
...
/// command may be executed; see [`crate::exec::is_natively_executable`].
pub fn validate(
    workspace: &Workspace,
    command: &str,
    cwd: Option<&str>,
...
mod tests {
    ...
    fn a_split_failure_is_reported_as_shlexs_own_error() {
        ...
        assert_eq!(err.kind, crate::sandbox::ErrorKind::Shlex);
