...
use crate::parse::pystr::{is_truthy, py_list_repr, py_str};
    ...
    async fn git_checkout(&self, path: &str) -> Value {
        ...
        match spawned {
            Ok(Ok(output)) => json!({
                ...
                "stderr": crate::parse::pystr::char_slice(
...
mod tests {
    ...
    use crate::kernel::agentloop::SNAPSHOT_MAX_BYTES;
    ...
    fn a_snapshot_of_an_escaping_path_says_so_instead_of_reading_it() {
        ...
        let workspace =
            crate::tools::sandbox::Workspace::new(dir.path().join("agent_workspace")).expect("ws");
        std::fs::write(dir.path().join("outside.txt"), b"secret").expect("outside");
        let runtime = Runtime::new(crate::kernel::agentloop::LoopDeps::new(
            crate::surface::worker::WorkerClient::new("http://127.0.0.1:1"),
            workspace,
        ));
        assert_eq!(
