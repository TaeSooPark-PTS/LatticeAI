//! Fixture replay + OpenAPI contract for peer network (WP-R9).
...
use lattice_platform::network::MOUNTED;
...
fn network_routes_match_the_committed_contract() {
    ...
    assert_mounted_matches_fragment(MOUNTED, &spec, |op| op.contains("/network/"));
}
