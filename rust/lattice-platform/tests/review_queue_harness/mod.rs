...
use lattice_platform::change_proposals::{self, check_and_apply_file};
...
async fn spawn_fake_worker(agent_root: PathBuf) -> String {
    async fn apply(
        ...
        if args.get("apply").and_then(Value::as_bool) != Some(true) {
            return (StatusCode::OK, axum::Json(json!({"decision": "none"}))).into_response();
        }
        let path = args.get("path").and_then(Value::as_str).unwrap_or("");
        let kind = args
            .get("kind")
            .and_then(Value::as_str)
            .unwrap_or("file_update");
        match check_and_apply_file(&root, &args, path, kind) {
            Ok(()) => (
                StatusCode::OK,
                axum::Json(json!({"applied": true, "path": path, "kind": kind})),
            )
                .into_response(),
            Err(change_proposals::ProposalConflict::Conflict(detail)) => {
                (StatusCode::CONFLICT, axum::Json(detail)).into_response()
            }
            Err(other) => (
                StatusCode::BAD_REQUEST,
                axum::Json(json!({"detail": format!("{other:?}")})),
            )
                .into_response(),
        }
    }
    ...
    let app = Router::new()
        .route("/agent/change-proposal", post(apply))
