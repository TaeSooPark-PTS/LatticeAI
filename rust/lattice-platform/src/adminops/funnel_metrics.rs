...
use crate::adminops::admin::{json_ok, now_iso};
...
impl FunnelService {
    ...
    pub fn snapshot(&self) -> OrderedMap {
        ...
        out.insert("counters", admin::json_from_ordered(&counters));
        out.insert("firsts", admin::json_from_ordered(&firsts));
        out.insert("rates", admin::json_from_ordered(&rates));
...
fn funnel_alerts(counters: &Map<String, Value>, rates: &Map<String, Value>) -> Vec<Value> {
    ...
    if ingest > 0 && recall == 0 {
        ...
        alerts.push(admin::json_from_ordered(&a));
    }
    alerts
}
