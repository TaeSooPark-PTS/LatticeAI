...
use crate::adminops::admin::{
    build_sensitivity_report, classify_sensitive_message, detail_status, json_ok, json_status,
    now_iso, redact_secret_text, redact_structure, tz_name,
};
...
pub(crate) async fn security_overview(
    ...
    out.insert("cards", admin::json_from_ordered(&cards));
    out.insert(
        "field_counts",
...
pub(crate) async fn security_conversation_summary(
    ...
    let items: Vec<Value> = history(&state)
        ...
        .enumerate()
        .map(|(i, h)| admin::json_from_ordered(&classify_sensitive_message(h, i)))
        .collect();
    let risky = items
        .iter()
        .filter(|it| {
