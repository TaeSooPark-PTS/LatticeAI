...
include!(concat!(
    ...
    "/src/toolsurface/generated_catalogs.rs"
));
...
pub use dispatch::{dispatch, dispatch_for_agent, is_native_tool, DispatchError};
pub(crate) use http::{
...
/// Streamable-HTTP MCP JSON-RPC. Also listed first in [`MOUNTED`] so the
/// OpenAPI product contract names the path the gateway actually serves.
...
impl McpState {
    ...
    pub fn with_tools(mut self, tools: tools::ToolsState) -> Self {
