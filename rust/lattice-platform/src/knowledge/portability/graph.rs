...
use crate::workspaceos::project_sessions::{detail, json_ok, parse_json_object};
...
fn export_artifact(state: &PortabilityState) -> OrderedMap {
    ...
    ordered.insert("exported_at", json!(project_sessions::now_iso_utc()));
...
fn write_backup_zip(state: &PortabilityState, dest: Option<&str>) -> Result<OrderedMap, String> {
    ...
    manifest.insert("created_at", json!(project_sessions::now_iso_utc()));
    manifest.insert(
        "db_sha256",
...
fn restore_backup_zip(
    ...
    if extracted {
        // Close idle Store/GraphWriter fds *before* the rename so a WAL
        // checkpoint cannot land on the incoming snapshot, then bump the
        // generation after the swap so the next checkout reopens.
        if let Some(graph) = &state.graph {
            graph.store().discard_idle_handles();
        }
        replace_live_db(&state.config.graph_db_path(), &snapshot)?;
        if let Some(graph) = &state.graph {
            graph.store().bump_generation();
        }
    }
    let _ = std::fs::remove_file(&snapshot);
