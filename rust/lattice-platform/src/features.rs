...
use crate::admin::{append_audit_event, audit_log_path, json_ok, language_from, message_error};
...
impl FeatureService {
    ...
    fn render_choices(&self, def: &Definition, language: &str) -> Vec<Value> {
        def.choices
            ...
            .map(|choice| {
                ...
                crate::admin::json_from_ordered(&row)
            })
            .collect()
    }
    ...
    pub fn catalog(&self, language: &str) -> OrderedMap {
        ...
        let features: Vec<Value> = CATALOG
            .iter()
            .map(|d| crate::admin::json_from_ordered(&self.render(d, &stored, language)))
            .collect();
        let mut out = OrderedMap::new();
...
async fn set_feature(
    ...
    match state.service.set(
        ...
        Err(FeatureError::Invalid(detail)) => {
            ...
            Err(crate::admin::json_status(StatusCode::BAD_REQUEST, &body))
        }
    }
}
            ...
            Some(other) => {
                ...
                if other.is_null() {
                    ...
                    .unwrap_or_else(|| {
                        ...
                        crate::admin::json_status(StatusCode::UNPROCESSABLE_ENTITY, &body)
                    }))
                } else {
                    Ok(other.clone())
                }
