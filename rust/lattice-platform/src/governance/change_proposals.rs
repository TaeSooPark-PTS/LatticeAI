...
async fn list_proposals(
    ...
    let items = review_queue::list_review_items(
        &state,
        scope.as_deref(),
    ...
    .into_iter()
    .filter(|item| review_queue::map_str(item, "effective_status") == "pending")
    .map(review_queue::into_value)
    .collect::<Vec<_>>();
    ...
    body.insert("contract", review_queue::into_value(contract));
    Ok(json_ok(&body))
}
...
fn reject_dismiss(
    ...
    let updated = review_queue::transition_dismiss(
        state,
        item_id,
        scope,
    ...
    Ok(review_queue::into_value(review_item_raw_view(&updated)))
}

fn raw_pending(state: &GovernanceState, scope: Option<&str>, user: Option<&str>) -> Vec<Value> {
    let ids: Vec<String> =
        review_queue::list_review_items(state, scope, user, Some("change_proposal"))
            .into_iter()
            .filter(|item| review_queue::map_str(item, "effective_status") == "pending")
            .map(|item| review_queue::map_str(&item, "id").to_string())
            .collect();
    ids.into_iter()
...
pub async fn apply_proposal(
    ...
    admin::append_audit_event(
        &admin::audit_log_path(&state.data_dir),
        "change_proposal_applied",
        event,
    );
    let mut result = json!({
        "item": review_queue::into_value(view),
        "applied": true,
        "path": path,
        "kind": kind,
    });
...
fn mark_approved(
    ...
    review_queue::update_review_item(state, item_id, scope, "approve", |item| {
