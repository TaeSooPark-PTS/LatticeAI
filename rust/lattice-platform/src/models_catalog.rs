...
//!
//! `POST /engines/pull-model` is PORT_ORCH in scout (huggingface_hub). It is
//! mounted so the OpenAPI fragment stays a bijection; the live download is a
//! documented gap (auth + consent + identifier checks are native).
...
pub fn router(state: ModelsCatalogState) -> Router {
    Router::new()
        ...
        .route("/engines/pull-model", post(engines_pull_model))
        .with_state(state)
}
...
async fn engines_pull_model(
    State(state): State<ModelsCatalogState>,
    headers: HeaderMap,
    body: Bytes,
) -> Result<Response, Response> {
    let parsed = parse_model(&body, &[required("model")])?;
    // allow_download is a bool with default false — parse by hand so a missing
    // field is false rather than a 422.
    let allow_download = serde_json::from_slice::<Value>(&body)
        .ok()
        .and_then(|value| value.get("allow_download").and_then(Value::as_bool))
        .unwrap_or(false);
    require_admin(&state, &headers)?;
    if !allow_download {
        return Err(localized(
    ...
    let model_ref = parsed.str("model").trim();
    ...
    let _ = resolve_language(&headers);
    // Live huggingface_hub / ollama pull stays on the worker. Auth + consent
    // + identifier checks are the native surface.
    Err(detail_error(
        StatusCode::NOT_IMPLEMENTED,
        "model pull remains on the Python worker (huggingface_hub / ollama)",
    ))
}
