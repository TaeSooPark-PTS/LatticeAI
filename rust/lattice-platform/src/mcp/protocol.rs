...
//!
//! This route is mounted on the product router but is **not** listed in
//! [`super::MOUNTED`]: it is outside the OpenAPI product contract. Session
//! auth is the same `require_user` every other product route uses (loopback
//! no-auth installs resolve to the local owner, which is what a localhost
//! client such as Claude Desktop sees).
