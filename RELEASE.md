...
- **Changed (Verification Contract)**: `verification.verified` now requires HF presence plus config and tokenizer hints, while weights presence is exposed separately as `has_weights_hint`; registry-only candidates stay off the load-ready catalog.
