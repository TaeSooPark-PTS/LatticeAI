...
const server = http.createServer((req, res) => {
  ...
  if (pathname === "/models") return json(res, {
    recommended: [
      ...
      },
      {
        ...
        runtime_label: "MLX-VLM with MLX-LM fallback",
        engine_options: [
          { engine: "local_mlx", model_id: "mlx-community/gemma-4-12b-it-4bit", load_id: "mlx-community/gemma-4-12b-it-4bit", runtime_label: "MLX-VLM with MLX-LM fallback" },
        ...
        runtime_compatibility: {
          ...
          user_message: "The installed MLX-VLM runtime does not expose the optional Gemma 4 unified drafter. Lattice will still try the v3 MLX load path and can fall back to MLX-LM or Gemma 4 GGUF local runtimes if needed.",
          recovery_guidance: ["Try the v3 MLX Gemma 4 load path first.", "Use MLX-LM or Gemma 4 GGUF if the MLX-VLM route fails."],
          alternatives: [
            { id: "mlx-community/gemma-4-12b-it-4bit", name: "MLX-LM", engine: "local_mlx" },
  ...
  if (pathname === "/models/recommendations") return json(res, {
    ...
    recommendations: {
      ...
      apple_silicon: true,
      ram_gb: 64,
      counts: { recommended: 2, compatible: 0, not_recommended: 0 },
      ...
      models: [
        ...
        { id: "mlx-community/gemma-4-12b-it-4bit", name: "Gemma 4 12B Instruct", family: "Gemma 4", status: "recommended", reason: "현재 메모리에서 안정적으로 사용할 가능성이 높습니다", size: "7.6GB", runtime_compatibility: { supported: true, status: "fallback_available", preferred_runtime: "MLX-VLM with MLX-LM fallback" } },
      ],
    },
  });
