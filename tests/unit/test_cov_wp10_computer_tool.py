...
# ── computer_open_app ────────────────────────────────────────────────────────
def test_open_app_requires_a_name():
    with pytest.raises(ToolError) as excinfo:
        computer.computer_open_app("   ")

    assert "앱 이름" in str(excinfo.value)
...
@pytest.mark.parametrize(
    ...
    [
        ("Darwin", ["open", "-a", "Notes"]),
        ("Windows", ["cmd", "/c", "start", "", "Notes"]),
        ("Linux", ["xdg-open", "Notes"]),
    ],
)
def test_open_app_uses_the_platform_launcher(monkeypatch, platform, expected):
    calls = []
    monkeypatch.setattr(computer, "_PLATFORM", platform)
    monkeypatch.setattr(computer, "subprocess", _fake_subprocess(calls))

    assert computer.computer_open_app("Notes") == {"action": "open_app", "app": "Notes"}
    assert calls[0][0] == expected


def test_open_app_surfaces_launcher_stderr(monkeypatch):
    calls = []
    monkeypatch.setattr(computer, "_PLATFORM", "Darwin")
    monkeypatch.setattr(
        computer, "subprocess",
        _fake_subprocess(calls, returncode=1, stderr=b"app not found"),
    )

    with pytest.raises(ToolError) as excinfo:
        computer.computer_open_app("Nope")

    assert "app not found" in str(excinfo.value)


# ── computer_open_url ────────────────────────────────────────────────────────
def test_open_url_requires_a_url():
    with pytest.raises(ToolError) as excinfo:
        computer.computer_open_url("  ")

    assert "URL이 필요" in str(excinfo.value)


def test_open_url_adds_https_and_targets_the_named_app(monkeypatch):
    calls = []
    monkeypatch.setattr(computer, "_PLATFORM", "Darwin")
    monkeypatch.setattr(computer, "subprocess", _fake_subprocess(calls))

    result = computer.computer_open_url("example.com", "Safari")

    assert calls[0][0] == ["open", "-a", "Safari", "https://example.com"]
    assert result == {"action": "open_url", "app": "Safari", "url": "https://example.com"}


def test_open_url_without_app_uses_the_default_handler(monkeypatch):
    calls = []
    monkeypatch.setattr(computer, "_PLATFORM", "Darwin")
    monkeypatch.setattr(computer, "subprocess", _fake_subprocess(calls))

    result = computer.computer_open_url("localhost:4825/app", "")

    assert calls[0][0] == ["open", "localhost:4825/app"]  # loopback keeps its scheme-less form
    assert result["app"] == "default"


@pytest.mark.parametrize(
    ("platform", "expected"),
    [
        ("Windows", ["cmd", "/c", "start", "", "https://example.com"]),
        ("Linux", ["xdg-open", "https://example.com"]),
    ],
)
def test_open_url_uses_the_platform_opener(monkeypatch, platform, expected):
    calls = []
    monkeypatch.setattr(computer, "_PLATFORM", platform)
    monkeypatch.setattr(computer, "subprocess", _fake_subprocess(calls))

    computer.computer_open_url("https://example.com", "Google Chrome")

    assert calls[0][0] == expected


def test_open_url_surfaces_opener_stderr(monkeypatch):
    calls = []
    monkeypatch.setattr(computer, "_PLATFORM", "Linux")
    monkeypatch.setattr(
        computer, "subprocess",
        _fake_subprocess(calls, returncode=2, stderr=b"no handler"),
    )

    with pytest.raises(ToolError) as excinfo:
        computer.computer_open_url("https://example.com")
